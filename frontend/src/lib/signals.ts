...
export async function expireOldSignals(): Promise<number> {
  // Time-based expiry is dishonest — we can't know if a trade won or lost without checking actual prices.
  // Only checkAndUpdateExpiredSignals() marks Won/Lost when real price hits TP or SL.
  // This function exists as a no-op so old calls don't break.
  return 0;
}
